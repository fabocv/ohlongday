...
TARGETS = ['estres', 'animo', 'claridad', 'activacion']

def parse_datetime(df: pd.DataFrame) -> pd.DataFrame:
    """Crea columna 'dt' robusta a formatos variados de 'hora' (sin helpers externos)."""
    import re as _re
    df = df.copy()
    ...
    def _norm(tok: str) -> str:
        ...
        if s in ("", "nan", "none"):
            return ""
        # normaliza separadores y sufijos
        s = s.replace("a. m.", "am").replace("p. m.", "pm")
        s = s.replace("-", ":").replace(".", ":").replace(";", ":").replace(",", ":")
        s = _re.sub(r"\s+", "", s)
        # compactos: 0930, 093005, etc.
        if _re.fullmatch(r"\d{3,6}", s) and ":" not in s and "am" not in s and "pm" not in s:
            if len(s) == 3:   # HMM -> H:MM
                s = f"{s[0]}:{s[1:]}"
                ...
                s = f"{s[:2]}:{s[2:4]}:{s[4:]}"
        # si no hay am/pm, pad a HH:MM:SS
    ...
    if hora_raw is not None:
        hora_norm = hora_raw.apply(_norm)
    ...
    # fallback: solo fecha si hora no parsea
...
def add_otras_sustancias_flags(df: pd.DataFrame) -> pd.DataFrame:
    """Crea flags 0/1 por palabras clave en 'otras_sustancias'."""
    df = df.copy()
    base = df.get("otras_sustancias")
    if base is None:
        # Si no existe la columna, crea las flags en 0
        for col in OTRAS_SUSTANCIAS_KEYWORDS.keys():
            df[col] = 0
        return df
    low = base.fillna("").astype(str).str.lower()
    for col, words in OTRAS_SUSTANCIAS_KEYWORDS.items():
        mask = low.apply(lambda s: any(w in s for w in words))
...
def plot_obs_vs_ema(df: pd.DataFrame, target: str, export_dir: str):
    """Genera un PNG con Observado vs EMA14 para el target."""
    try:
        os.makedirs(os.path.join(export_dir, "plots"), exist_ok=True)
        fig = plt.figure()
        ax = fig.gca()
        # series
        if "dt" in df.columns:
        ...
        y_obs = pd.to_numeric(df.get(target), errors="coerce")
        y_ema = pd.to_numeric(df.get(f"{target}_ema"), errors="coerce") if f"{target}_ema" in df.columns else None
        # plot
        ax.plot(x, y_obs, label=f"{target} observado")
        if y_ema is not None:
            ax.plot(x, y_ema, label=f"{target} EMA14")
        ax.set_title(f"{target.capitalize()} — Observado vs EMA14")
        ax.set_xlabel("Tiempo")
        ax.set_ylabel(target.capitalize())
        ax.legend()
        fig.autofmt_xdate()
        out_path = os.path.join(export_dir, "plots", f"{target}_obs_vs_ema.png")
        fig.savefig(out_path, bbox_inches="tight", dpi=150)
        plt.close(fig)
    except Exception as e:
        print(f"[AVISO] No se pudo generar el plot de {target}: {e}")
...
def time_series_splits(n: int, n_splits: int = 5, min_train: int = 30, test_size: int = 7):
    """
...
def evaluate_models_timecv(X: pd.DataFrame, y: pd.Series, df: pd.DataFrame, cat_cols: List[str]) -> Tuple[pd.DataFrame, Dict[str, object]]:
    ...
    # Ajuste para datasets pequeños: define tamaños de ventana dinámicos
    n = len(X)
    dyn_min_train = max(5, min(30, int(n * 0.6)))  # al menos 60% o 5 filas
    dyn_test_size = max(2, int(n * 0.2))           # al menos 20% o 2 filas
    # Definir pipelines
    pipe_en = Pipeline([
        ("pre", pre),
    ...
    splits = time_series_splits(n, n_splits=5, min_train=dyn_min_train, test_size=dyn_test_size)

    # Tercer modelo: HGBR (tolera NaNs)
    pipe_hgbr = Pipeline([
        ("pre", pre),
        ("hgbr", HistGradientBoostingRegressor(max_depth=None, learning_rate=0.1, max_iter=300, random_state=42))
    ])

    for name, pipe in [("ElasticNetCV", pipe_en), ("RandomForest", pipe_rf), ("HGBR", pipe_hgbr)]:
        fold_results = []
        for (tr0, tr1, te0, te1) in splits:
            Xtr, Xte = X.iloc[tr0:tr1], X.iloc[te0:te1]
            ytr, yte = y.iloc[tr0:tr1], y.iloc[te0:te1]

            if len(Xtr) < dyn_min_train or len(Xte) < dyn_test_size:
                continue
    ...
    best_model = models[best_name]
        ...
        # Baselines (Naive lag-1 y EMA14)
    baseline_rows = []

    def _fold_metrics_from_series(y_true: pd.Series, y_pred: pd.Series) -> Tuple[float, float]:
        m_mae = mean_absolute_error(y_true, y_pred)
        m_mse = mean_squared_error(y_true, y_pred)
        return float(m_mae), float(m_mse ** 0.5)

    # Reconstruir splits con n actual
    splits2 = time_series_splits(n, n_splits=5, min_train=dyn_min_train, test_size=dyn_test_size)

    if len(splits2) == 0:
        # Fallback holdout único
        split = int(max(5, n * 0.8))
        splits2 = [(0, split, split, n)]

    # Series alineadas
    y_shift1 = y.shift(1)
    ema_col = f"{y.name}_ema"
    y_ema14 = df.loc[y.index, ema_col] if ema_col in df.columns else pd.Series(index=y.index, dtype=float)

    # Naive y EMA: promediar métricas por fold
    fold_mae_naive, fold_rmse_naive = [], []
    fold_mae_ema, fold_rmse_ema = [], []
    for (tr0, tr1, te0, te1) in splits2:
        yte = y.iloc[te0:te1]
        pred_naive = y_shift1.iloc[te0:te1]
        pred_ema = y_ema14.iloc[te0:te1]
        # elimina pares con NaN
        mask_n = (~yte.isna()) & (~pred_naive.isna())
        if mask_n.any():
            m_mae, m_rmse = _fold_metrics_from_series(yte[mask_n], pred_naive[mask_n])
            fold_mae_naive.append(m_mae); fold_rmse_naive.append(m_rmse)
        mask_e = (~yte.isna()) & (~pred_ema.isna())
        if mask_e.any():
            m_mae, m_rmse = _fold_metrics_from_series(yte[mask_e], pred_ema[mask_e])
            fold_mae_ema.append(m_mae); fold_rmse_ema.append(m_rmse)

    if fold_mae_naive:
        metrics = pd.concat([metrics_df, pd.DataFrame([{
            "model": "BaselineNaive", "mae": float(np.mean(fold_mae_naive)), "rmse": float(np.mean(fold_rmse_naive)),
            "n_splits": len(fold_mae_naive), "n_samples": n
    ...
    if fold_mae_ema:
        metrics = pd.concat([metrics, pd.DataFrame([{
            "model": "BaselineEMA14", "mae": float(np.mean(fold_mae_ema)), "rmse": float(np.mean(fold_rmse_ema)),
            "n_splits": len(fold_mae_ema), "n_samples": n
        }])], ignore_index=True).sort_values("mae")

    # Re-escoge mejor modelo (no baseline) para continuar
    best_row = metrics[~metrics["model"].str.contains("Baseline")].sort_values("mae").iloc[0]
    best_name = best_row["model"]
    best_model = models[best_name]

    return metrics, { "best_name": best_name, "best_model": best_model, "all_models": models }
...
def _quantile_bands(series: pd.Series, low=0.15, high=0.85) -> Tuple[float, float]:
    s = pd.to_numeric(series, errors="coerce").dropna()
    if s.empty:
        return (np.nan, np.nan)
    return (float(s.quantile(low)), float(s.quantile(high)))

def _format_semáforo(val: float, qlow: float, qhigh: float) -> str:
...
def _mk_report_line(target: str, obs: float, ema14: float, pred_delta: float, obs_delta: float, sem: str, top_pos: list, top_neg: list) -> str:
    ...
    pos = ", ".join(top_pos[:3]) if top_pos else "—"
    neg = ", ".join(top_neg[:3]) if top_neg else "—"
    return (
...
def drivers_del_dia(df: pd.DataFrame, target: str, model: Pipeline, top_k: int = 5) -> Dict[str, object]:
    ...
    return {"texto": texto, "tabla": contribs_df, "top_pos": top_pos['feature'].tolist(), "top_neg": top_neg['feature'].tolist(), "delta_pred": float(delta_pred), "delta_obs": (float(delta_obs) if not math.isnan(delta_obs) else np.nan)}
...
def build_and_evaluate(df: pd.DataFrame, export_dir: str = "./outputs") -> Dict[str, object]:
    ...
    # Flags de 'otras_sustancias'
    df = add_otras_sustancias_flags(df)
# 3) EMA (para métricas continuas y mediadores)
    ...
    for target in TARGETS:
        ...
        # Plot Observado vs EMA14
        try:
            plot_obs_vs_ema(df, target, export_dir)
    ...
    # 9) Reporte diario combinado
    report_lines = ["# Reporte Coach Diario (comparativo vs ayer y EMA14)\n"]
    for target in TARGETS:
        if target not in df.columns or target not in best_models:
            continue
        # valores de hoy
        obs = df[target].iloc[-1] if not df[target].empty else np.nan
        ema14 = df.get(f"{target}_ema", pd.Series(dtype=float)).iloc[-1] if f"{target}_ema" in df.columns else np.nan
        # semáforo por cuantiles
        ql, qh = _quantile_bands(df[target])
        estado = _format_semáforo(obs, ql, qh)
        # drivers del día (recalcular por si acaso)
        expl = drivers_del_dia(df, target, best_models[target], top_k=5)
        line = _mk_report_line(target, obs, ema14, expl.get("delta_pred", np.nan), expl.get("delta_obs", np.nan), estado, expl.get("top_pos", []), expl.get("top_neg", []))
        report_lines.append(line)

    with open(os.path.join(export_dir, "reporte_coach.txt"), "w", encoding="utf-8") as f:
        f.write("\\n".join(report_lines))
